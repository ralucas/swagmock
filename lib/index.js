...
const mockOperation = (resolved, opsObj, mock, options) => {
    ...
    if (options.mockRequest) {
        ...
        if (options.testTemplates) {
            opsObj.responses = opsObj.responses && Object.keys(opsObj.responses).reduce((agg, res) => {
                if (Number(res) >= 200 && Number(res) < 400) {
                    agg[res] = opsObj.responses[res];
                }
                return agg;
            }, {});
            let successResponse = Object.keys(opsObj.responses)[0];
            if (successResponse) {
                mock[successResponse] = mockRequest(
                    resolved, 
                    mock.parameters || mockParameters(resolved, opsObj, options),
                    opsObj, 
                    options
